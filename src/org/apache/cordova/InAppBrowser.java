...
_L6:
        callbackcontext = parseFeature(jsonarray.optString(2));
        Log.d("InAppBrowser", (new StringBuilder()).append("target = ").append(s).toString());
        jsonarray = updateUrl(s2);
        if (!"_self".equals(s)) goto _L8; else goto _L7
_L7:
        Log.d("InAppBrowser", "in self");
        if (!jsonarray.startsWith("file://") && !jsonarray.startsWith("javascript:") && !Config.isUrlWhiteListed(jsonarray)) goto _L10; else goto _L9
_L9:
        webView.loadUrl(jsonarray);
        jsonarray = status;
        s = s1;
_L11:
        boolean flag;
        ...
        return true;
_L10:
        flag = jsonarray.startsWith("tel:");
        if (!flag)
        {
            break MISSING_BLOCK_LABEL_317;
        }
        s = new Intent("android.intent.action.DIAL");
        s.setData(Uri.parse(jsonarray));
        cordova.getActivity().startActivity(s);
        s = s1;
        jsonarray = status;
        continue; /* Loop/switch isn't completed */
        s;
        LOG.e("InAppBrowser", (new StringBuilder()).append("Error dialing ").append(jsonarray).append(": ").append(s.toString()).toString());
        s = s1;
        jsonarray = status;
        continue; /* Loop/switch isn't completed */
        s = showWebPage(jsonarray, callbackcontext);
        jsonarray = status;
        continue; /* Loop/switch isn't completed */
_L8:
        if (!"_system".equals(s))
    ...
    public String showWebPage(final String url, HashMap hashmap)
    {
        ...
        url = new Runnable() {
            ...
            
            {
                ...
                ((WebSettings) (obj3)).setDatabaseEnabled(true);
                ((WebSettings) (obj3)).setDatabasePath(cordova.getActivity().getApplicationContext().getDir("inAppBrowserDB", 0).getPath());
